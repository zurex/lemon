...
extern char *yytext;
extern MessageFormat len_compile_error_message_format[];
extern MessageFormat len_runtime_error_message_format[];
...
self_check()
{
    if (strcmp(len_compile_error_message_format[0].format, "dummy") != 0) {
        DBG_panic(("compile error message format error.\n"));
    }
    if (strcmp(len_compile_error_message_format
    ...
    if (strcmp(len_runtime_error_message_format[0].format, "dummy") != 0) {
        DBG_panic(("runtime error message format error.\n"));
    }
    if (strcmp(len_runtime_error_message_format
...
void
len_compile_error(CompileError id, ...)
{
    va_list     ap;
    VString     message;
    int         line_number;
    ...
    self_check();
    va_start(ap, id);
    line_number = len_get_current_interpreter()->current_line_number;
    clear_v_string(&message);
    format_message(&len_compile_error_message_format[id],
                   &message, ap);
...
void
len_runtime_error(int line_number, RuntimeError id, ...)
{
    va_list     ap;
    VString     message;
    ...
    va_start(ap, id);
    clear_v_string(&message);
    format_message(&len_runtime_error_message_format[id],
                   &message, ap);
    fprintf(stderr, "%3d:%s\n", line_number, message.string);
    va_end(ap);
